...
const ProfitLossPage = () =>{
    ...
    const column =  [
        ...
        {fieldName : 'description', title : '',render:(rowData,data)=><span className={styles.linkText} onClick={()=>showAccountStatement({data:rowData})}>{rowData?.game_id && data || "-"}</span>},
    ...
    const showAccountStatement = ({data}) =>{
        router.push({
            pathname: `/account-statement-detail`,
            query: {session_id:data?.session_id, market_id : data?.market_id, game_type_id : data?.game_type_id, game_id : data?.game_id, is_evo :data?.is_evo, is_vivo_bet :data?.is_vivo_bet, is_spade:data?.is_spade, is_ezugi : data?.is_ezugi},
        });
    }
